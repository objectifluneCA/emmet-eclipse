package io.emmet.eclipse.internal.templates;
...
public class ContributionContextTypeRegistry extends ContextTypeRegistry {
	...
	private static final Logger LOGGER = Logger.getLogger(ContributionContextTypeRegistry.class);

	/* extension point string literals */
	private static final String TEMPLATES_EXTENSION_POINT = "io.emmet.eclipse.templates"; //$NON-NLS-1$
	 ...
	 * contributions to the <code>io.emmet.eclipse.templates</code>
	 ...
	 * <code>io.emmet.eclipse.templates</code> extension point are
	...
	private static TemplateContextType createContextType(IConfigurationElement element) throws CoreException {
		...
		try {
			...
			throw new CoreException(new Status(IStatus.ERROR, EclipseEmmetPlugin.PLUGIN_ID, IStatus.OK,
					"extension does not implement " + TemplateContextType.class.getName(), e)); //$NON-NLS-1$
		}
	}
	...
	private static TemplateVariableResolver createResolver(IConfigurationElement element) throws CoreException {
		try {
			...
			throw new CoreException(new Status(IStatus.ERROR, EclipseEmmetPlugin.PLUGIN_ID, IStatus.OK,
					"extension does not implement " + TemplateVariableResolver.class.getName(), e)); //$NON-NLS-1$
		}
